...
from contrastive import builder_reward
from contrastive import config as contrastive_config
from contrastive import distributed_layout_reward
...
class DistributedContrastiveReward(distributed_layout_reward.DistributedLayoutReward):
  ...
  def __init__(
    ...
    contrastive_builder = builder_reward.ContrastiveBuilderReward(config,
                                                     logger_fn=logger_fn,
                                                     # save_data=save_data,
                                                     save_data=False,
    ...
    if evaluator_factories is None:
      ...
      evaluator_factories = [
          distributed_layout_reward.default_evaluator_factory(
    ...
    super().__init__(
        ...
        log_to_bigtable=log_to_bigtable,
        actor_logger_fn=distributed_layout_reward.get_default_logger_fn(
            log_to_bigtable, log_every, self._logdir, self._wandblogger),
        observers=actor_observers,
        checkpointing_config=distributed_layout_reward.CheckpointingConfig(directory=self._logdir, max_to_keep=config.max_checkpoints_to_keep, add_uid=False),)
