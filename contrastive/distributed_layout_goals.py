  ...
  def learner(
    ...
    if "offline" in self._builder._config.env_name:
        ...
        if "kitchen" in self._builder._config.env_name:
            episode_files = [file for file in episode_files if "initial_states.npz" not in file]
        ...
        if "kitchen" in self._builder._config.env_name:
            tasks_list = self._builder._config.env_name.split("-")[-1].split("+")
            assert len(tasks_list) == 1
            reward_key = "reward " + tasks_list[0]
        else:
            reward_key = "reward"

        print("reward_key:", reward_key)
        ...
        for ep_idx, episode_file in tqdm.tqdm(enumerate(episode_files), total=len(episode_files), desc="Loading episode files"):
            ...
            assert len(episode["observation"]) == len(episode["step_type"]) == len(episode["action"])  == len(episode["discount"]) == len(episode[reward_key])
            ...
            if expert_goals is None and episode[reward_key].sum() > 0:
                if "success" not in episode.keys() or "kitchen" in self._builder._config.env_name:
                    success_idxs = np.nonzero(episode[reward_key])[0]
                else:
                    success_idxs = np.nonzero(episode["success"])[0]


                # In the case of RPL_chopped_all, only select success examples from
                # chopped episode files where the target task is the task being solved.
                if "kitchen" not in self._builder._config.env_name or tasks_list[0] in episode_file:
